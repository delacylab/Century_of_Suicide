...
########################################################################################################################
# Overview: This script provides the functions needed to sample the individuals of a certain demographic group from the
# Multiple Cause of Death (MCOD) data (e.g., gender, race, generation, degree of rurality).
########################################################################################################################
...
def extract_sex(df: pd.DataFrame,
                gender: Literal['M', 'F']):
    """
    ...
    df_sub: pd.DataFrame = df[df['Gender'] == gender]
    return df_sub
...
def extract_3_race(df: pd.DataFrame,
    ...
    df_sub: pd.DataFrame = df[df['Race_Recode_3'] == race_int]
    return df_sub
...
def extract_5_race(df: pd.DataFrame,
    ...
    # Subset the records by the specific race
    df_sub: pd.DataFrame = df[df['Race_Recode_5'] == race_int]
    return df_sub
...
def extract_generation(df: pd.DataFrame,
    ...
    df_sub: pd.DataFrame = df[df['Generation'] == generation_int]
    return df_sub

########################################################################################################################
# Define a function to extract records with individuals belong to a specific rurality/urbanicity group defined by the
# Rural-Urban Continuum Codes (RUCC).
# Remark: Assume that 'FIPS' exists in the MCOD data as a column to encode individuals' residential area as a FIPS code.
# Users should merge the 'State_FIPS' and 'County_FIPS' column to create a 'FIPS' column storing FIPS code as strings in
# of length 5 (e.g., '02910') before applying this function.
# This function will be updated soon.
########################################################################################################################


def extract_rurality(df: pd.DataFrame,
                     year: int,
                     rurality_int: Literal[0, 1, 2]):
    ...
